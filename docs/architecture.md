...
Key-pool cursors and cooldowns are process-local and concurrency-safe. Each request visits an eligible key at most once, with its ordinary bounded HTTP retries. A single provider concurrency lease covers the entire selection/failover and any stream. The pool never retries a successful HTTP response, including a body read or stream that later fails. Attempts persist credential fingerprints without storing API keys. Catalog reload creates pools for new requests while existing routes retain their pool and captured prices.
